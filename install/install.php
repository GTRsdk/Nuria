...
class Install{
	...
	function CreateConfigFile(){
		...
		if(file_exists('include/config.php')){
			...
			header('Location: index.php?step=2&error=2');
		}
		fclose($fh);
	}

	function CreateHtaccess(){
		$file = 'include/htaccess.php';
		$fh = fopen($file, 'w') or die('<br/><br/><br/>Can\'t open file: install\include\htaccess.php');
		$text = file_get_contents("include/htaccess.tpl");
		$text = preg_replace("'%SUBFOLDER%'", $_POST['subfolder'], $text);
		fwrite($fh, $text);
		if(file_exists('include/htaccess.php')){
			header('Location: index.php?step=4');
		}else{
			header('Location: index.php?step=3&error=3');
		}
		fclose($fh);
	}
	...
	function CreateServerFile(){
		...
		if(file_exists('include/izariam.php')){
			...
			header('Location: index.php?step=4&error=4');
		}
		fclose($fh);
	}
	...
	function ImportDB(){
		...
		if($result){
			...
			header('Location: index.php?step=5&error=5');
		}
	}
	...
	function ReplaceFiles(){
		...
		rename("include/htaccess.php", "../.htaccess");
		if (file_exists('../index.php') and file_exists('../izariam/config/database.php') and file_exists('../izariam/config/config.php') and file_exists('../izariam/config/izariam.php')) {
			header('Location: ../index.php');
		} else {
			header('Location: index.php?step=6&error=6');
		}
	}
};
$install = new Install;
